...
class Gallagher::AzureAPI < PlaceOS::Driver
  ...
  default_settings({
    # gallagher via Azure APIM
    azure_apim_subscription: "c02314",
    azure_tenant_id:         "d3f2b-d2d-a68-bd9-b98283",
  ...
  def on_load
    ...
    schedule.every(10.minutes) { connected }
  ...
  @azure_apim_subscription : String = ""
  ...
  def on_update
    ...
    @azure_apim_subscription = setting(String, :azure_apim_subscription)
    @azure_tenant_id = setting(String, :azure_tenant_id)
  ...
  def add_access_group_member(group_id : String | Int32, cardholder_id : String | Int32, from_unix : Int64? = nil, until_unix : Int64? = nil)
    from_time = Time.unix(from_unix) if from_unix
    until_time = Time.unix(until_unix) if until_unix
    group = CardholderAccessGroup.new(
      {href: "#{@custom_href_base || @uri_base}#{@access_groups_endpoint}#{group_id}/".as(String?), name: nil.as(String?)},
      from_time,
      until_time,
    )
