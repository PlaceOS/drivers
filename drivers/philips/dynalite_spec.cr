...
DriverSpecs.mock_driver "Philips::Dynalite" do
  ...
  should_send Bytes[0x1c, 0x02, 0x02, 0x71, 0x82, 0x32, 0xff, 0xbc]
  status["area2_chan3_level"].should eq(49.5)

  # 50 second fade
  exec :light_level, area: 2, level: 49.5, fade: 50000, channel: 3
  should_send Bytes[0x1c, 0x02, 0x02, 0x72, 0x82, 0x32, 0xff, 0xbb]
  status["area2_chan3_level"].should eq(49.5)

  # 15 minute fade
  exec :light_level, area: 2, level: 49.5, fade: 900000, channel: 3
  should_send Bytes[0x1c, 0x02, 0x02, 0x73, 0x82, 0x0f, 0xff, 0xdd]
  status["area2_chan3_level"].should eq(49.5)

  exec :stop_fading, area: 4, channel: 6
  should_send Bytes[0x1c, 0x04, 0x05, 0x76, 0x00, 0x00, 0xff, 0x66]
  ...
  should_send Bytes[0x1c, 0x02, 0x04, 0x61, 0x00, 0x00, 0xff, 0x7e]
  responds Bytes[0x1c, 0x02, 0x04, 0x60, 0x70, 0x70, 0xff, 0x9f]
  response.get.not_nil!.as_f.to_i.should eq(56)
  status["area2_chan5_level"].not_nil!.as_f.to_i.should eq(56)
end
